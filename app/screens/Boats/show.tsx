...
const Boat = ({navigation, route}: any) => {
  ...
  useEffect(() => {
    ...
    const getUsers = async () => {
      ...
      const usersPromises = boat.crew.map((user_id: string) =>
        getUser(user_id),
      );
  ...
  const showTabs = () => {
    switch (index) {
      case 0:
        return (
          <ScrollView>
            <Text>Length: {boat.length}</Text>
            <Text>Width: {boat.width}</Text>
            <Text>Draft: {boat.draft}</Text>
            <Text>Weight: {boat.weight}</Text>
            <Text>Sail Area: {boat.sail_area}</Text>
            <Text>Engine: {boat.engine}</Text>
            <Text>Fuel: {boat.fuel}</Text>
            <Text>Water: {boat.water}</Text>
